...
describe("Graphics module", function()
    ...
    describe("Window", function()
        ...
        it("Window::create", function()
            assert.has_no_errors(function() window:create(100, 100, 'Test', Graphics.Window.Style.Windowed) end)
            assert.has_errors(function() window:create(0, 100, 'Test', Graphics.Window.Style.Windowed) end)
        end)
    end)
    ...
    describe("Shader class", function()
        ...
        it("Shader::loadFromFile", function()
            assert.is_true(shader:loadFromFile('tests/test.vs', 'tests/test.fs'))
            assert.is_true(shader:isLoaded())
        end)
    end)
    ...
    describe("Texture class", function()
        ...
        it("Texture:loadFromFile()", function()
            assert.is_true(texture:loadFromFile('tests/test.png'))
            assert.are_equal(texture.size, Zadymka.Math.Vec2u:new(640, 640))
            texture.size.x = 10
            print(texture.size.x)
        end)
        it("Texture:loadFromMemory()", function()
            local file = io.open('tests/test.png', 'rb')
